...
class form_date_class extends form_custom_class
{
	...
	Function AddInput(&$form, $arguments)
	{
		...
		$this->hour=$this->GenerateInputID($form, $this->input, "hour");
		$this->minute=$this->GenerateInputID($form, $this->input, "minute");
		$this->second=$this->GenerateInputID($form, $this->input, "second");
		...
		$hour_arguments=array(
			"NAME"=>$this->hour,
			"ID"=>$this->hour,
			"TYPE"=>"select",
			"OPTIONS"=>$month_options,
		);
		$minute_arguments=array(
			"NAME"=>$this->minute,
			"ID"=>$this->minute,
			"TYPE"=>"select",
			"OPTIONS"=>$month_options,
...
*/
		...
		|| strlen($error=$form->AddInput($day_arguments)))
			return($error);
		if (1==1) {
			if(strlen($error=$form->AddInput($hour_arguments))
			|| strlen($error=$form->AddInput($minute_arguments))
			|| strlen($error=$form->AddInput($second_arguments)))
				return($error);
		}
		return("");
	}
	...
	Function LoadInputValues(&$form, $submitted)
	{
		if($this->choose_control)
		{
			...
			if(!$this->choose != !$choose)
			{
				...
				$form->SetInputProperty($this->hour, 'ExtraAttributes', $value);
				$form->SetInputProperty($this->minute, 'ExtraAttributes', $value);
				$form->SetInputProperty($this->day, 'ExtraAttributes', $value);
				$this->choose = $choose;
			}
		}
