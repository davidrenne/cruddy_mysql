...
/*
 *
 * @(#) $Id: form_list_select.php,v 1.11 2010/02/02 09:52:11 mlemos Exp $
 *
 */
...
class form_list_select_class extends form_custom_class
{
	...
	Function AddInputPart(&$form)
	{
		...
		|| $this->multiple)
		{
			...
			for($o = 0, Reset($this->options); $o < $total; Next($this->options), ++$o)
			{
				...
				if(strlen($this->get_selected_option))
				{
					$checked = $form->GetJavascriptCheckedState('f', $this->name.$option);
					if(strlen($checked)==0)
				...
				if($this->multiple)
				{
					$checked = $form->GetJavascriptSetCheckedState('f', $this->name.$option, 'c');
					if(strlen($checked)==0)
	...
	Function GetInputValue(&$form)
	{
		if($this->multiple)
		{
			...
			{
				$option = Key($this->options);
				if($form->GetCheckedState($this->name.$option))
					$selected[] = $option;
			}
			return($selected);
		}
	...
	Function SetInputProperty(&$form, $property, $value)
	{
		switch($property)
		{
			case 'ONCHANGE':
				...
				{
					$option = Key($this->options);
					if(strlen($error = $form->SetInputProperty($this->name.$option, 'ONCHANGE', $value)))
						return($error);
				}
				return('');
			case 'VALUE':
				if(!$this->multiple)
				{
					...
					if(!IsSet($this->options[$this->VALUE = strval($value)]))
						return('it was not specified a valid list select value');
					if(strlen($error = $form->SetCheckedState($this->name.$this->VALUE, 1))
					|| strlen($error = $form->SetCheckedState($this->name.$old, 0)))
						return($error);
					return('');
				}
				break;
